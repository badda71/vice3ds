...
SDL_Surface *uibottom_scr=NULL;
SDL_Surface *kbd_img=NULL;
...
void uibottom_init(void)
{
	// init the 3DS bottom screen
	// bottomscreen image
	uibottom_scr = SDL_SetVideoMode(BOTTOMSCREEN_W, BOTTOMSCREEN_H, 32, SDL_SWSURFACE | SDL_BOTTOMSCR);
//	uibottom_scr = SDL_SetVideoMode(BOTTOMSCREEN_W, BOTTOMSCREEN_H, 32, SDL_SWSURFACE);
	SDL_FillRect(uibottom_scr, NULL, SDL_MapRGB(uibottom_scr->format, 0x30, 0x30, 0x30));
	if (kbd_img == NULL) {
		char *fname = archdep_join_paths(archdep_user_config_path(),"ui_keyboard.bmp",NULL);
		kbd_img = SDL_LoadBMP(fname);
		lib_free(fname);
	}
	kb_x_pos=(uibottom_scr->w-uikbd_pos[0][2])/2;
	kb_y_pos=uibottom_scr->h - uikbd_pos[0][3];
	...
	SDL_BlitSurface(kbd_img, &rsrc, uibottom_scr, &rdest);

    SDL_Flip(uibottom_scr);

	uibottom_kbdactive = 1;
}
